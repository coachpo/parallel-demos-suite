/* 
   ...
   program, for instance 4, 9 or 16 processes.
...
int main(int argc, char *argv[]) {
  ...
  if (q*q == ntasks) {
    ...
    /* Get own rank in column communicator */
    MPI_Comm_rank(col_comm, &id_col);
    
    printf("Process %d has rank %d in its row communicator and %d in the column communicator\n", id, id_row, id_col);
