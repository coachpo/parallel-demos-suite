...
the message and replies with a message containing the hostname of the
machine the process runs on. The sender process receives these messages
...
#include <stdio.h>
#include "mpi.h"
#define MAXPROC 384    /* Max number of procsses */
...
int main(int argc, char* argv[]) {
  ...
  char hostname[MAXPROC][MPI_MAX_PROCESSOR_NAME];  /* Array of received host names */
  int namelen;
  ...
  if (me == 0) {    /* Process 0 does this */
    ...
    for (int i=1; i<np; i++) {
      ...
      /* Request array starts from element 1 */
    ...
    /* Send own hostname back to process 0 */
