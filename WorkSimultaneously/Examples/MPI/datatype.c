...
three input values (one float, one double and one int) from standard input,
...
Compile the program with 'mpicc -O2 datatype.c -o datatype'
Run the program with 2 proceses.
...
int main(int argc, char *argv[]) {
  ...
  if (id == 0) {			       	/* Process 0 does this */
    ...
    fflush(stdout);
    scanf("%f %lf %d", &indata.a, &indata.b, &indata.n);  /* Read input */
  }
  
  lengtharray[0] = lengtharray[1] =lengtharray[2] = 1;  /* Set array of lengths */
  typearray[0] = MPI_FLOAT;                            /*   and data types */
  ...
  MPI_Get_address(&indata.a, &startaddress);
  MPI_Get_address(&indata.b, &address);
  disparray[1] = address-startaddress;     /* Displacement of second element, b */

  MPI_Get_address(&indata.n, &address);
  disparray[2] = address-startaddress;     /* Displacement of third element, n */

  /* Build the data structure my_type */
  MPI_Type_create_struct(3, lengtharray, disparray, typearray, &my_type);
  MPI_Type_commit(&my_type);

  if (id==0) {       /* Process 0 does this */

    printf("The size of the structure Indata_type is %d\n", sizeof(Indata_type));
    
    // Print out displacements (look at the alignment of the second element!)
    printf("      disparray is ");
    for (int i=0; i<3; i++) {
      printf (" %ld ", (unsigned long int)disparray[i]);
    }
    printf("\n");
    ...
    printf("Received message with values %3.1f  %3.1lf  and %d from process %d\n", \
