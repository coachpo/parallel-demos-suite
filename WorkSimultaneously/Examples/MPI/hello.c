...
int main(int argc, char *argv[]) {
  ...
  if (id == 0) {
    ...
    /* Receive and print greetings from all other processes */
    for (i=1; i<ntasks; i++) {
      /* Receive a message from any sender */
      if ( MPI_Recv(msg, 80, MPI_CHAR, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status) != MPI_SUCCESS) {
  ...
  /* Processes 1 to N-1 (the senders) do this */
    ...
    int length;
    MPI_Get_processor_name(msg, &length);  /* Get name of this processor */
    if ( MPI_Send(msg, length, MPI_CHAR, 0, tag, MPI_COMM_WORLD) != MPI_SUCCESS) {
