...
operations (MPI_Isend/MPI_Irecv and MPI_Waitall) to wait until the
...
int main(int argc, char* argv[]) {
  ...
  MPI_Status status[MAXPROC];                        // Status objects
  MPI_Request send_req[MAXPROC], recv_req[MAXPROC];  // Request objects
  ...
  int x = me;   /* This is the value we send, the process id */
  ...
  if (me == 0) {    /* Process 0 does this */
    ...
    printf("Pocess %d sent messages to all other\n", me);
